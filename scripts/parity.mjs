...
// Judged on take: gaming win (theoretical) ÷ money the crowd brought, so wealth and market size don't decide it.
...
for (const t of crowds) {
  ...
  for (const seed of String(seeds).split(",").map(Number)) {
    ...
    for (let d = 0; d < days; d++) { counting = d >= warm; for (let k = 0; k < sim.TICKS_PER_DAY; k++) g.step(); g.bus.flush(); }
  ...
  rows.push({ t, take: acc.theo / Math.max(1, acc.brought), broughtVisit: acc.brought / Math.max(1, acc.visits), theoDay: acc.theo / dd, winDay: acc.win / dd, perVisit: acc.theo / Math.max(1, acc.visits), edge: acc.theo / Math.max(1, acc.wag),
...
const cols = [["crowd", (r) => sim.GUEST_TYPES[r.t].name], ["take", (r) => `${(100 * r.take).toFixed(1)}%`], ["per $1K brought", (r) => usd(1000 * r.take)], ["brought/visit", (r) => usd(r.broughtVisit)], ["gaming/day (theo)", (r) => usd(r.theoDay)], ["actual", (r) => usd(r.winDay)],
...
// Parity is judged on take (gaming win ÷ money the crowd brought: wallets and ATM draws), so a crowd's wealth and
// market size don't decide it: with markets sized to the same spending power, the same take is the same income.
const th = rows.map((r) => r.take), lo = Math.min(...th), hi = Math.max(...th);
const flags = [];
if (hi > 2 * lo) flags.push(`take spread ${(hi / Math.max(1e-9, lo)).toFixed(1)}× (${rows.find((r) => r.take === hi).t} highest, ${rows.find((r) => r.take === lo).t} lowest): goal is roughly equal`);
