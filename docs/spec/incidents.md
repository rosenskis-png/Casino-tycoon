...
- **Being policed**: a warning adds 8 × `policed` annoyance ("Buzzkill."). Thrown out: the visit score × 0.4, the person's `ejects` count +1 (bans are M5), and each companion gets 8 × `policed` annoyance ("They threw my friend out!"). A thrown-out leader takes the group.
...
- **Regulator**: standing shown (100), with no triggers until M5/M8.
