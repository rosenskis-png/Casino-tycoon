...
for (let d = 0; d < 365; d++) {
  for (let t = 0; t < sim.TICKS_PER_DAY; t++) {
    g.step();
    if (t === 0) { n++; guests += g.state.agents.filter((a) => a.role === "guest").length; }
  }
  g.bus.flush();
}
const cats = ["slots", "bar", "drinks", "wages", "upkeep"];
console.log(`average guests on the floor ${(guests / n).toFixed(0)}; cash at year end ${Math.round(g.state.cash)}`);
