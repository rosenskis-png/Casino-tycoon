...
import { CABINETS, TOPPERS, type SlotDesign } from "../../data/designer";
...
export function designPrice(d: SlotDesign): { cost: number; upkeep: number } {
  const cost = CABINETS[d.cab.type].cost + TOPPERS[d.cab.topper].cost + (d.fs ? 50 : 0) + 25 * d.jackpots.length;
