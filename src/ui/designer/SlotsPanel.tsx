    ...
    <>
      ...
      {lib.length ? lib.map((e) => (
        <div key={e.key} className="dz-list-row">
          ...
          <button className="btn" onClick={() => { play("click"); open(sanitize({ ...e.d, id: "" })); }}>Use</button>
      ...
      <div className="dz-code" style={{ marginTop: 8 }}>
        ...
        <button className="btn" onClick={() => { const d = parseCode(code); if (d) { setCode(""); open(sanitize({ ...d, id: "" })); } else play("deny"); }}>Open</button>
      </div>
    </>
  );
}
