...
- (Batch A, owner: "a bunch of bathrooms and everyone's still complaining about lines") The lines were a bug, not capacity: restrooms on the Test Floor were used ~35 times each in 60 days, but a guest whose known restrooms were full complained and wandered off at every decision (1,545 "line" thoughts in 60 days). Now, when every restroom they know is full, a guest first looks a couple of steps for one they haven't found (while it isn't urgent), then waits by the nearest one, grumbling once per wait; after a long wait at 95+ they go home. A stall visit is 4–7 s (was 5–9). Test Floor, 60 days: 41 line thoughts.
...
- **Per-bar policy** (tap a bar): price multiplier 0–3× on the $7 base; comped share 0–100% (for guests who've played); strength Light 0.6 / Standard 1 / Strong 1.4; and where its servers work: anywhere, or any set of rooms ticked on the bar's card (Batch C, owner; rooms are named in the room inspector). Each drink costs the house $1.50 ("Drink costs"). Pricey or weak drinks draw complaints.
...
- Incident fields in type data (M4, docs/spec/incidents.md): `incidents` and `tolerance` per category, `policed`, `drama`. `cheat` share (M5, docs/spec/cheats.md). Cheats never look any different.
...
Batch A Test Floor (no luck, restroom lines fixed; seed 1, 300 days): visit length 6.5 / 6.7 / 6.2 / 6.9 / 10.3 / 5.5 / 8.0 min (Locals / Retirees / Tourists / Party / High rollers / Families / Conventioneers), playing 2.6 / 1.8 / 1.1 / 0.5 / 4.1 / 0.0 / 1.9, visit score 0.60 / 0.59 / 0.63 / 0.59 / 0.80 / 0.81 / 0.64; reputation after 300 days 66 / 61 / 69 / 65 / 62 / 74. Flags: only Stampede Gold barely played (as on main).
