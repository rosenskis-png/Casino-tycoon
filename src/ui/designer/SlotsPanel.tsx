...
export function SlotsPanel({ g, open, place }: { g: Game; open: (d: SlotDesign) => void; place: (id: string) => void }) {
  ...
  // (M8.6) The library remembers a design's sale and Evergreen status.
  useEffect(() => {
    let lib: ReturnType<typeof loadLibrary> | null = null;
    for (const [id, rec] of Object.entries(s.designs)) {
      const ever = s.dstats[id]?.ever ? 1 as const : undefined;
      if (rec.sale || ever) lib = noteInLibrary((d) => d.name.replace(/ v\d+$/, "") === rec.d.name.replace(/ v\d+$/, "") && sameMath(sanitize(d), rec.d), { sold: rec.sale?.maker, peak: rec.sale?.peak, ever });
    }
    if (lib) setLib(lib);
  }, [s]);
    ...
    return (
      ...
      <div className="dz-list-row">
        <div>
          ...
          {marketLine(g, id) && <small>{marketLine(g, id)}</small>}
        </div>
        {st && st.born >= 0 && <button className="btn" onClick={() => { play("click"); setMore(more === id ? null : id); }}>{more === id ? "Less" : "More"}</button>}
      ...
      {more === id && <div className="mk-more"><DesignMarket g={g} id={id} /></div>}
      </Fragment>
    );
  };
  return (
    <>
      ...
      <RecordsAndWishes g={g} />
      <p className="muted" style={{ margin: "10px 0 6px" }}>Library (this phone)</p>
      {lib.length ? lib.map((e) => (
        <div key={e.key} className="dz-list-row">
          <div><b>{e.d.name}</b><small>{LAYOUTS[e.d.layout]?.name} · {(e.d.rtp * 100).toFixed(1)}%{e.ex !== undefined ? ` · Excitement ${e.ex.toFixed(1)} · Intensity ${e.int?.toFixed(1)}` : ""}</small>
            {(e.sold || e.ever) && <small>{[e.ever ? "Evergreen" : "", e.sold ? `Sold to ${e.sold}${e.peak ? `, peak ${e.peak.toLocaleString("en-US")} machines` : ""}` : ""].filter(Boolean).join(" · ")}</small>}</div>
